...
import { fetchMaterialRequestedById, updateMaterialById } from "services/dashboard/material";
...
export default function EditMaterialRequested(){

    const materialRequestedData:any = useLoaderData();
    console.log("material",materialRequestedData)
    ...
    const [materialRequestedList,setMaterialRequestedList]:any = useState(materialRequestedData.material)
        ...
        <Form method="post" className="w-full h-full mt-5">
            <div>
                ...
                <div className="flex mt-4 flex-row gap-3">
                    <Input type="text" name="series" defaultValue={materialRequestedData.ID} labelPlacement="outside" variant="faded" label="Series" />
                    <Input type="text" name="transactionDate" defaultValue={materialRequestedData.transactionDate} labelPlacement="outside" variant="faded"  label="Transaction Date"  />
                    <Input type="text" name="purpose" defaultValue={materialRequestedData.purpose}  labelPlacement="outside" variant="faded" label="Purpose"  />
                </div>
                <div className="flex flex-row mt-4 gap-3">
                    <Input type="text" name="requiredBy" defaultValue={materialRequestedData.requiredBy} labelPlacement="outside" variant="faded"   label="Required By" />
                    <Input type="text" name="company" defaultValue={materialRequestedData.company} labelPlacement="outside"  variant="faded"  label="Company"/>
                </div>
            </div>
            <br />
            <div className="flex flex-col pt-10 mt-5">
                ...
                <div className="w-full mt-4">
                    <div className="flex flex-row gap-3">
                        <Input type="text" name="setTargetWarehouse" defaultValue={materialRequestedData.setTargetWarehouse} label="Set Target Warehouse" variant="faded"   labelPlacement="outside" />
...
export async function loader({params}:LoaderFunctionArgs){
    const materialRequestedId = await params.selectedMaterialRequested;
    if(materialRequestedId){
        const materialRequestedData = await fetchMaterialRequestedById(materialRequestedId);
...
export async function action({request,params}:ActionFunctionArgs) {
    ...
    if(materialRequestedId){
        const isUpdated = await updateMaterialById(data,materialRequestedId);
        if(isUpdated){
